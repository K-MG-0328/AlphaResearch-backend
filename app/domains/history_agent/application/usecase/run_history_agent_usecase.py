...
from app.domains.history_agent.application.service.timeline_enrichment_service import (
    TimelineEnrichmentService,
)
...
# ── 공시 중복 제거 / lookback 윈도우 / region 매핑 ────────────────
# 모듈 분리 (C1 step 1): _announcement_dedup, _lookback_window 로 이동.
# 본 파일에서는 import 후 동일 심볼명으로 재노출하여 외부 호출(테스트 포함) 호환을 유지.
from app.domains.history_agent.application.usecase._announcement_dedup import (  # noqa: E402, F401
    _ANNOUNCEMENT_DEDUP_THRESHOLD,
...
from app.domains.history_agent.application.usecase._lookback_window import (  # noqa: E402, F401
    _CHART_INTERVAL_LOOKBACK_DAYS,
    _DEFAULT_CHART_INTERVAL_LOOKBACK_DAYS,
    _DEFAULT_INDEX_REGION,
    _ETF_REGION,
    _INDEX_REGION,
)
...
# _PERIOD_DAYS / datetime_date_from_period 는 _lookback_window 로 이동. 호환 위해 재노출.
from app.domains.history_agent.application.usecase._lookback_window import (  # noqa: E402, F401
    _PERIOD_DAYS,
    datetime_date_from_period,
)
...
class RunHistoryAgentUseCase:
    ...
    def __init__(
        ...
        self._enrichment_service = TimelineEnrichmentService(
            enrichment_repo=enrichment_repo,
            event_impact_repo=event_impact_repo,
        )
    ...
    # 4 메서드 본체는 TimelineEnrichmentService 로 이동 (C1 step 2).
    # 외부 호출자(테스트 포함) 호환을 위해 thin wrapper 보존.
    # service 의 의존성(enrichment_repo, event_impact_repo) 은 __init__ 시점에
    # UseCase 가 받은 동일 인스턴스를 위임하므로 mock 기반 단위 테스트 동작 동일.

    async def _apply_event_impact_metrics(
        self, ticker: str, timeline: List[TimelineEvent]
    ) -> None:
        await self._enrichment_service.apply_event_impact_metrics(ticker, timeline)

    async def _load_enrichments(
        self, ticker: str, timeline: List[TimelineEvent]
    ) -> Dict[Tuple, "EventEnrichment"]:
        return await self._enrichment_service.load_enrichments(ticker, timeline)
    ...
    def _apply_enrichments(
        ...
        return self._enrichment_service.apply_enrichments(ticker, timeline, db_map)

    async def _save_enrichments(self, ticker: str, events: List[TimelineEvent]) -> None:
        await self._enrichment_service.save_enrichments(ticker, events)
